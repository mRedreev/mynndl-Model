// cars_loader.js
// Парсинг UCI-подобного cars CSV, где пропуски помечены '?'.
// Поддерживаем разделители ',' и ';'. Целевая колонка — 'price' (строго по имени).
...
export async function parseCarsCSV(file) {
  ...
  if (!('price' in col)) throw new Error("Не найдена колонка 'price'");
